...
export function HomeBlogCard({ section: data = section }) {
  return (
    <article className="group flex h-full w-full flex-col justify-between gap-4">
      ...
      <Link
        href={data.url}
        className="group-hover:bg-primary-red inline-flex w-fit items-center gap-2 rounded-sm bg-black px-8 py-2 text-lg font-bold text-white transition duration-500 ease-in-out"
      >
        {data.ctaLabel}
        <ArrowRight size={24} />
      </Link>
