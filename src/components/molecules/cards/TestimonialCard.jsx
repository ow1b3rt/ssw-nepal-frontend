  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 p-6">
        <h3 className="text-primary-blue-dark text-2xl font-extrabold">{data.title}</h3>
      ...
      >
        <ImageContainer
          className="h-16 w-16 rounded-full border-2 border-white grayscale-0"
          src={data.author.avatar}
          alt={data.author.name}
        />
        <div className="flex flex-col">
          <span className="text-xl font-semibold">{data.author.name}</span>
          <span className="text-base font-semibold opacity-80">{data.author.since}</span>
        </div>
      </div>
    </div>
  );
