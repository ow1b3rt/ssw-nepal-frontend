...
import { FaCaretUp, FaTrash, FaPlus, FaGripVertical } from "react-icons/fa";
import { setPath } from "@/packages/admin";

function FaqItemEditable({ number, item, path, onChange, open, onToggle, onRemove, dropProps, dragHandleProps }) {
    ...
    >
      <div ref={headerRef} className="flex w-full items-center gap-4 px-4 py-3">
        ...
        >
          <FaCaretUp
            size={18}
            className={`cursor-pointer transform transition-transform duration-300 ${open ? "rotate-0" : "rotate-180"}`}
          />
        </span>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="btn btn-primary rounded-lg bg-black cursor-pointer hover:bg-black/80 px-6 py-2 font-bold text-white disabled:opacity-60"
        >
