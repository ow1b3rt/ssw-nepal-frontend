  ...
  return (
    <Form defaults={defaults} id={externalId} onSubmit={onSubmit} className="flex flex-col gap-6">
      <div className="flex flex-col gap-6 lg:flex-row">
        <div className="flex min-w-0 flex-2 flex-col gap-4 rounded-xl border border-gray-200 bg-white p-6">
          {left.map((field) => (
        ...
        {right.length > 0 && (
          <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-4 lg:flex-1">
            {right.map((field) => (
              <PayloadField key={field.name?.split(":")?.[0]} field={field} />
            ))}
          </div>
